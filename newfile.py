...
g= datetime.datetime(2024,5  , 1, 12, 00 ,0)
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s{ [جاري الصيد] %s/%s ➙ [صحيح] %s ➙ [سكيور] %s .. } %s%s%s'%  (bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
بدايه لطيفه حب لا تفقد الامل
▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱▱
.
➩ : @GG6TG
					'''
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='مايشتغل'))
...
➩ : @GG6TG
					...
					cetak(nel(statusok1, title='شغال بلعافيه'))
